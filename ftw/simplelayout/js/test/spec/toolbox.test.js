...
describe('Toolbox', function() {
  ...
  it('default layouts are [1, 2, 4]', function() {
    let defaultToolbox = new Toolbox()
  ...
  it('can enable and disable blocks', function() {
    ...
    assert.equal(
      'sl-toolbox-blocks disabled',
      toolbox.element.find('.sl-toolbox-blocks')[0].className
    )
  ...
  describe('addables', function() {
    it('can render blocks', function() {
      ...
      assert.deepEqual(
        $.map(toolbox.element.find('.sl-toolbox-block'), function(block) {
          return {
      ...
      assert.deepEqual(
        $.map(toolbox.element.find('.sl-toolbox-blocks .description'), function(description) {
    ...
    it('can render layouts', function() {
      ...
      assert.deepEqual(
        $.map(toolbox.element.find('.sl-toolbox-layout'), function(layout) {
      ...
      assert.deepEqual(
        $.map(toolbox.element.find('.sl-toolbox-layout .description'), function(description) {
          return { description: description.innerHTML }
        }),
    ...
    it('can render icon', function() {
      ...
      assert.deepEqual(
        $.map(toolbox.element.find('.sl-toolbox-blocks .icon-textblock'), function(icon) {
    ...
    it('does not reder layout when canChangeLayout is disabled', function() {
      ...
      assert.equal(
        toolbox.element.find('.sl-toolbox-layouts').length,
        0,
        'Layouts should not get rendered'
      )
    })
  })
})
