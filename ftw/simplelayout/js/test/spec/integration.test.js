...
describe('Integration', function() {
  ...
  it('creates toolbar on block with given blockactions from toolbox', function(done) {
    ...
    simplelayout.on('toolbar-attached', block => {
      expect(
        $.map(block.element.find('.sl-toolbar-block a'), function(action) {
  ...
  it('does not create toolbar for layouts if layout edit is inactive', function() {
    ...
    assert.deepEqual(
      $.map(layout.element.find('.sl-toolbar-layout a'), function(action) {
        return action.className
      }),
      []
    )
  ...
  it('a layout should only have its configured actions', () => {
    ...
    assert.deepEqual(
      $.map(layout2.element.find('.sl-toolbar-layout a'), function(action) {
        return action.className
      }),
    ...
    assert.deepEqual(
      $.map(layout4.element.find('.sl-toolbar-layout a'), function(action) {
        return action.className
      }),
      ['edit', 'move', 'all']
    )
  ...
  it('an action with no rules should be applied to all layouts', () => {
    ...
    assert.deepEqual(
      $.map(layout1.element.find('.sl-toolbar-layout a'), function(action) {
        return action.className
      }),
      ['edit', 'move', 'all']
    )
