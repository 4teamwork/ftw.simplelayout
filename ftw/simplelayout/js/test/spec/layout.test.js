...
describe('Layout', function() {
  ...
  it('each column of 4 column layout has sl-col-4 class', function() {
    let nodes = $.map(layout.element.find('.sl-column'), function(column) {
  ...
  describe('Block-transactions', function() {
    it('can insert a block', function() {
      let generatedBlock = layout.insertBlock('<p></p>', 'textblock')
      let blocks = $.map(layout.blocks, function(block) {
    ...
    it('can delete a block', function() {
      let generatedBlock = layout.insertBlock()
    ...
    it('can move a block to another layout', function() {
      let generatedBlock = layout.insertBlock('<p>Test</p>', 'textblock').commit()
      let target = new Layout()
      layout.moveBlock(generatedBlock, target)

      assert.equal(layout.getCommittedBlocks(), 0, 'Sourcelayout shoult not have any blocks')
      assert.deepEqual(
        [{ id: generatedBlock.id, committed: true, parent: target.id }],
    ...
    it('block stores parent information', function() {
      let generatedLayout = new Layout()
      let generatedBlock = generatedLayout.insertBlock()
  ...
  describe('set content', function() {
    ...
    it('should restore all blocks containing the layout', function(done) {
      ...
      layout.content(`<div class='sl-block'>Hallo</div><div class='sl-block'>Velo</div>`)
    })

    it('should restore the layout toolbar', done => {
      EE.on('toolbar-attached', layout => {
        expect(layout.element.find('.sl-toolbar-layout').length).toEqual(
          1,
          'The layout toolbar was not restored.'
        )
        done()
      })
  ...
  describe('Block accessors', function() {
    it('can get committed blocks', function() {
      let block2 = layout.insertBlock()
      layout.insertBlock()
      layout.insertBlock()
      let blocks = $.map(layout.getCommittedBlocks(), function(block) {
    ...
    it('can get inserted blocks', function() {
      ...
      layout.insertBlock()
      layout.insertBlock()
      let blocks = $.map(layout.getInsertedBlocks(), function(block) {
        return block.committed
      })
    ...
    it('block can remove hinself from layout', function() {
      ...
      assert.deepEqual(
        $.map(layout.blocks, function(block) {
          return block.id
        }),
        []
      )
