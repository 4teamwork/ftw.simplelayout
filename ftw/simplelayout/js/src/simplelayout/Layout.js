...
export default function Layout(columns) {
  ...
  this.getCommittedBlocks = function() {
    return $.grep(
      $.map(this.blocks, function(block) {
        return block
      }),
      function(block) {
  ...
  this.content = function(toReplace) {
    ...
    $('.sl-block', this.element).each(function() {
      _this.insertBlock().restore(this, _this, $(this).data().type, $(this).data().uid)
    })
  ...
  this.restore = function(
    restoreElement = this.element,
    restoreParent = this.parent,
    restoreColumn = this.columns,
